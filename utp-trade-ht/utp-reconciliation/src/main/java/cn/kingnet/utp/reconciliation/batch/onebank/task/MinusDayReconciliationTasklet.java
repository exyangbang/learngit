...
/**
 * 只针对交易记录超过N天未进行对账
 * 查找有对账文件，未对账过的，这些交易清算日期字段为空的，进行对账
 *
...
@Component
@Slf4j
public class MinusDayReconciliationTasklet implements Tasklet {

    @Value("${cn.kingnet.utp.reconciliation.minus-days}")
    ...
    public RepeatStatus execute(StepContribution contribution, ChunkContext chunkContext) throws Exception {
        try {
            ...
            //获取前N天的日期
            final String minusDate = LocalDate.parse(reconcileDate, DateTimeFormatter.BASIC_ISO_DATE).minusDays(minusDays).format(DateTimeFormatter.BASIC_ISO_DATE);
            Map<String, Object> parameterMap = new HashMap<>(4);
            parameterMap.put("minusDate", minusDate);
            log.info("开始处理 于对账日期[{}]{}天前[{}]的交易日期未对账的记录且渠道清算日期为空的记录", reconcileDate, minusDays, minusDate);
            List<TransCurrent> list = MybatisDaoImpl.of(TransCurrentMapper.class).selectListBySql("getMinusDayTransList", parameterMap);
            if (!CollectionUtils.isEmpty(list)) {
                AtomicLong tradeFailureCount = new AtomicLong();
                List<SettleCheckInfo> listInserts = list.stream().filter(transCurrent -> {
                    //过滤渠道清算日期字段不为空==[即不处理]
                    if (!StringUtil.isBlank(transCurrent.getChannelSettleDate())) {
                        return false;
                    }
                    //原渠道手续费
                    ...
                    transCurrent.setDescription(String.format("%s;[处理N天未进行对账-本地多流水]对账前交易状态(%s)", StringUtil.trimToEmpty(transCurrent.getDescription()), transCurrent.getTransStatus()));
                    ...
                    //过滤掉失败的交易记录==[即超过N天依然未对账，则业务上判定为 明确失败，不进差错表直接移至到历史表]
                     ...
                     */
                    return getTSettleCheckInfo(transCurrent, ReconciliationErrorType.F, "平台多流水", TrueOrFalseStatus.FALSE.value(), "需要运营人员人工介入处理", origTransStatus, reconcileDate);
                }).collect(Collectors.toList());
                int size = listInserts == null ? 0 : listInserts.size();
                log.info("{}天前，当前清算日期[{}]，以平台为基础对账 ==>  有{}笔差错交易流水记录", minusDays, reconcileDate, size);
                if (size > 0) {
                    MybatisDaoImpl.run().insertBatch(listInserts, 100);
                }
                MybatisDaoImpl.run().updateBatchById(list, 100);

                // 超过N天依然未对账，则业务上判定为 明确失败，不进差错表直接移至到历史表
                //交易失败并且是需要进入历史表标识的记录，才迁移到历史流水表
                if (tradeFailureCount.get() > 0) {
                    Map<String, Object> failMoveParams = Maps.newHashMap();
                    failMoveParams.put("reconcileDate", reconcileDate);
                    failMoveParams.put("minusDate", minusDate);
                    failMoveParams.put("transStatus", TransStatus.TRADE_FAILURE.name());
                    MybatisDaoImpl.of(TransHistoryMapper.class).insertBySql("moveTradeFromCurrent4MinusDays", failMoveParams);
                    MybatisDaoImpl.of(TransCurrentMapper.class).deleteBySql("deleteCurrentDate4MinusDays", failMoveParams);
                    log.info("对账日期[{}] >> 超过[{}]天未对账的失败交易状态被判定为明确失败，直接迁移至历史流水表. 迁移记录数:{}", reconcileDate, minusDays, tradeFailureCount.get());
                }
            }
            log.info("处理完成 于对账日期[{}]{}天前[{}]的交易日期未对账的记录且渠道清算日期为空的记录", reconcileDate, minusDays, minusDate);
        } catch (Exception e) {
            throw new ReReconciliationTaskletException("以平台为基础处理N天前未对账处理异常：".concat(e.getMessage()), e);
        }
