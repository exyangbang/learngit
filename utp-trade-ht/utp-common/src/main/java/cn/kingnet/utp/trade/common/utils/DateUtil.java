...
public class DateUtil {
	...
	public static final String FORMAT_DADETIME = "yyyyMMddHHmmss";
	public static final String FORMAT_TRADEDATE = "yyyyMMdd";
	public static final String FORMAT_TRADETIME = "HHmmss";
	...
	public static DateTimeFormatter BASE_TODAY_FORMATTER = DateTimeFormatter.ofPattern(FORMAT_TRADEDATE);
	...
	public static String getDiffTime(String beginTime, String endTime) {
		try {
			...
			Date eTime = getDateFromString(endTime, "yyyy-MM-dd HH:mm:ss");
	...
	public static String getFulltime() {
		Calendar calendar = Calendar.getInstance();
		return getStringFromDate(calendar.getTime(), "yyyyMMddHHmmss");
	}
	...
	public static String getFulltimeByFormat() {
		Calendar calendar = Calendar.getInstance();
		return getStringFromDate(calendar.getTime(), "yyyy-MM-dd");
	}
	...
	public static int selectDateDiff(String first, String second) {
		...
		int dif;
		try {
			Date fDate = getDateFromString(first, "yyyy-MM-dd");
			Date sDate = getDateFromString(second, "yyyy-MM-dd");
			dif = (int) ((fDate.getTime() - sDate.getTime()) / 86400000L);
	...
	public static long selectMillSecDiff(String start, String end, String date_formate) {
		...
		try {
			...
			between = end_date.getTime() - begin_date.getTime();
	...
	public static boolean isYyyyMMdd(String yyyyMMdd) {
		...
		try {
			...
			throw new RuntimeException(String.format("非法的日期字符:%s", yyyyMMdd));
		}
	...
	public static boolean isHHmmss(String HHmmss) {
		...
		try {
			...
			throw new RuntimeException(String.format("非法的时间字符:%s", HHmmss));
		}
	 ...
	 * @return int 天数
	 */
	public static long betweenDays(String startDate, String endDate, DateTimeFormatter formatter) {
		return LocalDate.parse(startDate, formatter).until(LocalDate.parse(endDate, formatter), ChronoUnit.DAYS);
	}
	...
	public static Date parseDate(String dateStr, String pattern) {
		return Date.from(LocalDateTime.parse(dateStr, DateTimeFormatter.ofPattern(pattern)).toInstant(ZoneOffset.of("+8")));
	}

	/**
	 * @param date    日期对象
	 * @param pattern 格式化日期 格式
	 * @return 格式化字符串
	 */
	public static String formateDate(Date date, String pattern) {
		return LocalDateTime.ofInstant(date.toInstant(), ZoneOffset.of("+8")).format(DateTimeFormatter.ofPattern(pattern));
	}
	 ...
	 * @return 日期对象
	 */
	public static Instant parseDateForInstant(String dateStr, String pattern) {
		return LocalDateTime.parse(dateStr, DateTimeFormatter.ofPattern(pattern)).toInstant(ZoneOffset.of("+8"));
	}
	...
	public static String getCurrentTimeSecond() {
		DateFormat df = new SimpleDateFormat(FORMAT_DADETIME);
		return df.format(new Date());
	}
	 ...
	 * 两个时间是否相差N分钟
	 */
	public static Boolean isBetweenMinus(String start, String end, int defaultMinute) {
		long minutes = Duration.between(parseDateForInstant(start, FORMAT_DADETIME), parseDateForInstant(end, FORMAT_DADETIME)).toMinutes();
		if (minutes == defaultMinute - 1) {
			return true;
		}
		return false;
	}
	...
	public static String getToday(){
		return LocalDateTime.ofInstant(Instant.now(), ZoneId.systemDefault()).format(BASE_TODAY_FORMATTER);
	}
