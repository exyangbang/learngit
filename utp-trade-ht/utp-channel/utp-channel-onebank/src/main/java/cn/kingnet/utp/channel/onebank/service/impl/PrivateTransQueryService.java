...
public class PrivateTransQueryService extends BaseOnebankPayService<OneBankTransQueryReqDTO, OneBankTransQueryRespDTO> {
    ...
    private static final String PRIVATE_TRANS_QUERY_KEY = "privateTransQuery";
    private static final String API_NAME = UnionPaySubTypeEnum.PRIVATE_TRANS_QUERY.getDesc();
    @Resource
    ...
    protected ChannelResponseDTO syncReq(FetureContext<OneBankTransQueryReqDTO, OneBankTransQueryRespDTO> fetureContext) {
        ...
        log.info("流水号:{} >> [RDFA异步对私转账交易结果查询] 请求参数：==>>{}", oneBankTransQueryReqDTO.getClientTradeId(), JSON.toJSONString(oneBankTransQueryReqDTO));
        ...
        try {
            ...
            log.error("流水号:{} >> [RDFA异步对私转账交易结果查询] 调用渠道接口异常：==>>{}", oneBankTransQueryReqDTO.getClientTradeId(), e.getMessage(), e);
        ...
        if (ConstantsForProto.SYSTEM_FAIL.equals(rsp.getMessage().getSysHead().getTransactionReturnStatus())) {
            log.error("流水号:{} << [RDFA异步对私转账交易结果查询] 渠道响应系统报文头状态为失败 <<== {}", oneBankTransQueryReqDTO.getClientTradeId(), JSON.toJSONString(rsp.getMessage().getSysHead()));
            return ChannelResponseDTO.builder()
                    .serverTransId(oneBankTransQueryReqDTO.getClientTradeId())
        ...
        if (rsp.getMessage().getAppHead() != null && rsp.getMessage().getAppHead().getRet() != null) {
            ...
            log.info("流水号:{} << [RDFA异步对私转账交易结果查询] 响应报文体：{}", oneBankTransQueryReqDTO.getClientTradeId(), respBody == null ? null : respBody.toJSONString());
            ...
            String status = HttpRespStatus.OK.valueStr(), exceptionMsg = null;
            ...
            if (OneBankRespCode.PCC00000.name().equals(respCode)) {
                ...
                transStatus = TransStatus.TRADE_FAILURE.name();
                statusDesc = "交易失败";
                exceptionMsg = String.format("[%s] %s", StringUtil.trimToEmpty(respCode), StringUtil.trimToEmpty(respMsg));
            } else {
                status = HttpRespStatus.NOT_ACCEPTABLE.valueStr();
            ...
            log.error("流水号:{} << [RDFA异步对私转账交易结果查询] 渠道响应应用报文头appHead为空或报文头内容appHead.ret为空 <<== {}", oneBankTransQueryReqDTO.getClientTradeId(), rsp.getMessage().getAppHead() == null ? null : JSON.toJSONString(rsp.getMessage().getAppHead()));
            return ChannelResponseDTO.builder()
                    .serverTransId(oneBankTransQueryReqDTO.getClientTradeId())
                    .channelRespCode(rsp.getMessage().getSysHead().getTransactionReturnStatus())
    ...
    public SysHeader buildSysHeader(FetureContext<OneBankTransQueryReqDTO, OneBankTransQueryRespDTO> fetureContext) {
        ...
        /**目标dfa值 ：由华通提供*/
        sysHeader.setTargetDfa(UtpDMBUtil.getRDfaListSync());
        /**平台dfa值：由华通提供**/
        sysHeader.setDfa(DmbUtil.getConsumerDFA());
        /**服务代码，由华通提供*/
        sysHeader.setServiceId(this.onebankChannelProperties.getTransfer(PRIVATE_TRANS_QUERY_KEY).getServiceId());
        /**服务场景，默认01，由华通提供*/
        sysHeader.setSceneId(this.onebankChannelProperties.getTransfer(PRIVATE_TRANS_QUERY_KEY).getSceneId());
        /**全局业务流水号*/
