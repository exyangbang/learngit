...
public class AuthConfigInfoServiceImpl implements IAuthConfigInfoService {
    ...
    public AuthConfigInfo gainAuthConfigInfo(TradeContext tradeContext) {
        ...
        ValidateUtils.hasText(authNo, "平台客户号");
        try {
            ...
            if (CommonConstants.AUTHORIZATION_TYPE_HTSIGN.equals(utpConfigProperties.getSignType())) {
                ...
                Industry industry = basicConfigHandler.getIndustryByIndustryCode(authNo);
                authConfigInfo.setDn4CreditCode(industry != null ? industry.getDn() : "");
            } else {
            ...
            throw new TradeException(HttpRespStatus.NOT_FOUND, StringUtil.truncate("获取配置连接异常!" + e.getMessage(), 200));
        } catch (InvalidKeySpecException e) {
            throw new TradeException(HttpRespStatus.UNAUTHORIZED, StringUtil.truncate("公私钥信息有误，请联系管理员." + e.getMessage(), 200));
        } catch (RetryableException e) {
            throw new TradeException(HttpRespStatus.UNAUTHORIZED, StringUtil.truncate("公私钥信息有误，请联系管理员." + e.getMessage(), 200));
        } catch (TradeException e) {
            throw e;
        } catch (Exception e) {
            throw new TradeException(HttpRespStatus.SERVER_ERROR, "获取平台公私钥配置异常");
        }
    }
}
