...
import cn.kingnet.utp.trade.common.dto.account.UnfreezeOrderIdReqDTO;
import cn.kingnet.utp.trade.common.dto.account.UnfreezeOrderIdRespDTO;
...
@TradeService(
        ...
        tradeType = TradeType.UNFREEZE_ORDERID,
        reqClass = UnfreezeOrderIdReqDTO.class,
        respClass = UnfreezeOrderIdRespDTO.class
)
public class UnfreezeOrderIdService extends AbstractAccountTradeService<UnfreezeOrderIdReqDTO, UnfreezeOrderIdRespDTO> {

    @Resource
    private IBankAccountTransDetailHandler bankAccountTransDetailHandler;
    @Resource
    private IAccountOrderLogHandler accountOrderLogHandler;
    @Resource
    private IChannelRelevantHandler channelRelevantHandler;
    public UnfreezeOrderIdService(IAuthConfigInfoService authConfigInfoService, UtpConfigProperties utpConfigProperties, IBasicConfigHandler basicConfigHandler, ProductAccountProperties productAccountProperties) {
        super(authConfigInfoService, utpConfigProperties, basicConfigHandler, productAccountProperties);
    }


    @Override
    protected void verifyReqParam(TradeContext<UnfreezeOrderIdReqDTO, UnfreezeOrderIdRespDTO> tradeContext) {
        super.verifyReqParam(tradeContext);
        UnfreezeOrderIdReqDTO reqDTO = tradeContext.getRequestDTO();
    ...
    @Override
    public ChannelResponseDTO reqChannel(TradeContext<UnfreezeOrderIdReqDTO, UnfreezeOrderIdRespDTO> tradeContext) {

        UnfreezeOrderIdReqDTO reqDTO = tradeContext.getRequestDTO();
        UnfreezeOrderIdRespDTO.UnfreezeOrderIdRespDTOBuilder respDTOBuilder = UnfreezeOrderIdRespDTO.builder();
        ...
        try {
            ...
            if (orderStatus==null || orderStatus == AccountOrderStatus.INITTIALIZE) {
                throw new TradeException(HttpRespStatus.FORBIDDEN, "查无此订单金额冻结记录");
            }
            if (orderStatus != AccountOrderStatus.FREEZED) {
                throw new TradeException(HttpRespStatus.FORBIDDEN, String.format("该订单金额%s,不能再解冻",orderStatus.getDesc()));
            }
            ...
            builder.id(IdGenerate.getId()).clientTransId(tradeContext.getClientTradeId())
                    .serverTransId(tradeContext.getServerTransId()).remark("订单金额解冻")
                    .account(reqDTO.getUserAccount()).orderId(reqDTO.getOrderId()).amt(Long.valueOf(reqDTO.getPayAmt()))
                    .tradeType(TradeType.UNFREEZE_ORDERID.getCode()).transStatus(TransStatus.PRE_CREATE.name())
            ...
            this.accountHandler.unfreezeAccountBalance(AccountOperBo.builder().industryCode(basicConfigInfo.getIndustryCode())
                    .accountOperSubType(AccountOperSubType.ORDER_INCOME).userAccount(reqDTO.getUserAccount())
                    .amount(detail.getTxnAmount()).peerAccNo(detail.getOtherCustAccountNo()).peerAccName(detail.getOtherCustName())
                    .serverTransId(tradeContext.getServerTransId()).outsideOrderId(detail.getSysReferenceNo()).remark("订单金额解冻")
                    .build()

            );
            bankAccountTransDetailHandler.updateOrderStatusByOrderId(reqDTO.getUserAccount(),reqDTO.getOrderId(),AccountOrderStatus.UNFREEZED.getCode());

            builder.transStatus(TransStatus.TRADE_SUCCESS.name()).gmtModified(DateUtil.getSystemDate());
            accountOrderLogHandler.updateAccountOrderLog(builder.build().buildVo());

        } catch (Exception e) {
            throw new TradeException(HttpRespStatus.SERVER_ERROR, "订单金额解冻异常:" + e.getMessage(),e);
        }
        return ChannelResponseUtil.success(respDTOBuilder.build(), null, null);
    }

    @Override
    public void initTransFlow(TradeContext<UnfreezeOrderIdReqDTO, UnfreezeOrderIdRespDTO> tradeContext) {

    }

    @Override
    public void updateTransFlow(TradeContext<UnfreezeOrderIdReqDTO, UnfreezeOrderIdRespDTO> tradeContext) {
