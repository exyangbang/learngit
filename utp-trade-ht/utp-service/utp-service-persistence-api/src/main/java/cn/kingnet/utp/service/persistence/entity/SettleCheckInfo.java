...
import cn.kingnet.utp.service.persistence.entity.vo.SettleCheckInfoVo;
...
@Data
@Builder
@MapperInterface("cn.kingnet.utp.service.persistence.mapper.SettleCheckInfoMapper")
public class SettleCheckInfo extends BaseEntity {
    ...
    public SettleCheckInfoVo buildVo() {
        return new SettleCheckInfoVo(id, errorType, errorDesc, status, remark, accountHandleFlag, serverTransId, clientTransId, reconcileTime, reconcileDate, beforeTransStatus, transStatus, transAmount, merFee, channelFee, realAmount, merNo, industryCode, productCode, tradeType, transDate, transTime, channelKey, channelCode, channelName, channelRespCode, channelRespId, channelRespMsg, channelRespTime, clientTransTime, orgCode, orgName, serverBatchNo, serverLinkId, respCode, revokeStatus, refundStatus, userCode, batchSubNo, pcFlag, userType, payeeAcctNo, payeeName, payerAcctNo, payerName, accNo, gmtCreate, gmtModified, userCreate, userModified, merSettleType, merChargeMode, channelSettleType, channelChargeMode, channelSettleDate, orginChannelFee, payerAccount, payeeAccount, feeAccount, incomeAccount, needChannelReconcile, showPayerAccountNo, showPayerAccountName, showPayeeAccountNo, showPayeeAccountName, orderId, elcMakeFlag, feeAccountName, incomeAccountName, payerOpbkName, payeeOpbkName, summaryDescription, auditStatus);
    }

    public static SettleCheckInfo of(SettleCheckInfoVo vo) {
        return new SettleCheckInfo(vo.getId(), vo.getErrorType(), vo.getErrorDesc(), vo.getStatus(), vo.getRemark(), vo.getAccountHandleFlag(), vo.getServerTransId(), vo.getClientTransId(), vo.getReconcileTime(), vo.getReconcileDate(), vo.getBeforeTransStatus(), vo.getTransStatus(), vo.getTransAmount(), vo.getMerFee(), vo.getChannelFee(), vo.getRealAmount(), vo.getMerNo(), vo.getIndustryCode(), vo.getProductCode(), vo.getTradeType(), vo.getTransDate(), vo.getTransTime(), vo.getChannelKey(), vo.getChannelCode(), vo.getChannelName(), vo.getChannelRespCode(), vo.getChannelRespId(), vo.getChannelRespMsg(), vo.getChannelRespTime(), vo.getClientTransTime(), vo.getOrgCode(), vo.getOrgName(), vo.getServerBatchNo(), vo.getServerLinkId(), vo.getRespCode(), vo.getRevokeStatus(), vo.getRefundStatus(), vo.getUserCode(), vo.getBatchSubNo(), vo.getPcFlag(), vo.getUserType(), vo.getPayeeAcctNo(), vo.getPayeeName(), vo.getPayerAcctNo(), vo.getPayerName(), vo.getAccNo(), vo.getGmtCreate(), vo.getGmtModified(), vo.getUserCreate(), vo.getUserModified(), vo.getMerSettleType(), vo.getMerChargeMode(), vo.getChannelSettleType(), vo.getChannelChargeMode(), vo.getChannelSettleDate(), vo.getOrginChannelFee(), vo.getPayerAccount(), vo.getPayeeAccount(), vo.getFeeAccount(), vo.getIncomeAccount(), vo.getNeedChannelReconcile(), vo.getShowPayerAccountNo(), vo.getShowPayerAccountName(), vo.getShowPayeeAccountNo(), vo.getShowPayeeAccountName(), vo.getOrderId(), vo.getElcMakeFlag(), vo.getFeeAccountName(), vo.getIncomeAccountName(), vo.getPayerOpbkName(), vo.getPayeeOpbkName(), vo.getSummaryDescription(), vo.getAuditStatus());
    }
}