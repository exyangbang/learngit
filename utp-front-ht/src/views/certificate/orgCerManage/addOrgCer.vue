    ...
    <Form ref="formValidate" :model="formData" :label-width="120" :rules="ruleValidate" method="post" enctype="multipart/form-data">
      <Row>
        <Col span="10" offset="2">
          ...
          <!--<FormItem label="选择收单机构：" prop="orgCode">-->
            <!--<Select v-model="formData.orgCode" >-->
              <!--<Option v-for="item in this.funcTableData" :value="item.orgCode">{{ item.fullName }}</Option>-->
            <!--</Select>-->
          <!--</FormItem>-->
  ...
  methods: {
    ...
    handleSubmit (name) {
      ...
      self.$refs[name].validate((lidate) => {
        if (lidate) {
          this.formData.certificateType = '1'
          ...
          this.$axios.post(this.$contants.UploadPublicCertificateUrl, fd, config)
            .then(function (response) {
              self.$Message.success('添加客户证书成功')
              self.goBack()
            })
            .catch(function () {
            })
