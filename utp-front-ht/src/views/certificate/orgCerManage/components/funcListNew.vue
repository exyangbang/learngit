        ...
        <Row>
            <Col>
                <Form :label-width="100" ref="form" :model="formData" label-position="right" :inline="true">
                    <FormItem label="客户号：">
                        <Input v-model="formData.code" :maxlength="100"/>
                    </FormItem>
                    <FormItem label="客户名称：">
                        <Input v-model="formData.name" :maxlength="100"/>
                    </FormItem>
...
export default {
  ...
  methods: {
    ...
    searchList (param) {
      ...
      // 获取 商户列表(证书关联的商户)
      this.$axios.post(this.$contants.restful.industryInfoCers, param)
        .then((response) => {
          self.tableData = []
    ...
    sureSubmit: function () {
      ...
      self.selectedMerchant.forEach(function (currentPageValues, currentPageIndex) {
        currentPageValues.forEach(function (itemValues, index) {
          merchants.push(itemValues.code)
        })
      })
      let url = self.$contants.restful.certificateIndustryInfo + '/' + self.selectedItem.id
