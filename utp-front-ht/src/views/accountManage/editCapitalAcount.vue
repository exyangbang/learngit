        ...
        <Col span="24">
          <Col span="12">
            <FormItem label="资金账号：" prop="bankAccountNo">
              <Input v-model="formDataNew.bankAccountNo" :disabled="true"  style="width:350px" placeholder="请输入资金账号" :maxlength="accountSize" />
            </FormItem>
          </Col>
          <Col span="12">
          ...
          <Col span="12">
            <FormItem label="资金账号类型：" prop="accountType">
              <i-select v-model="formDataNew.accountType" :disabled="true" style="width:300px" @on-change="accountTypeChange" placeholder="请选择资金账号类型">
          ...
          <Col span="12">
            <FormItem label="账户用途：" prop="useType">
              <i-select v-model="formDataNew.useType" :disabled="true" style="width:300px" @on-change="useTypeChange" placeholder="请选择账户用途">
...
export default {
  ...
  methods: {
    ...
    getDetail () {
      let self = this
      this.$axios.get('/fundAccount' + '/' + this.id)
        .then(function (response) {
          if (response.data.shareFlag === '' || response.data.shareFlag === null) {
            response.data.shareFlag = '0'
          }
    ...
    handleSubmit () {
      ...
      this.$refs['formDataNew'].validate((valid) => {
        if (valid) {
          this.$axios.put('/fundAccount', this.formDataNew).then(function (res) {
            that.$Message.success('修改资金账户成功')
            that.goBack()
          })
        } else {
  ...
  activated () {
    let initParamVal = this.$util.getRefreshInitParam(this, 'editCapitalAcount', this.$route.params.id)
