            ...
            <Col span="12">
              <FormItem label="手续费模板：" prop="merFeeTemplateId">
                ...
                <i-input style="width: 350px;" v-model="choseText"  @on-focus="choseMerFeeTem" placeholder=choseText>
                  <!--<Input icon="ios-clock-outline" style="width: 300px;" v-model="formDataNew.chnFeeTemplateIdAry" :disabled="true" placeholder="请选择手续费模板" />-->
                  <!--<Input style="width: 350px;" v-model="formDataNew.merFeeTemplateId" placeholder="请选择手续费模板" />-->
                  <i-button slot="append" icon="ios-search" @click="choseMerFeeTem"></i-button>
                </i-input>
                <!--<i-select v-model="formDataNew.merFeeTemplateId" style="width:350px" @on-change="merFeeTemplateIdChange">-->
...
export default {
  ...
  methods: {
    ...
    getOrganizationDetail () {
      ...
      this.$axios.get('channelMerchant' + '/' + this.channelMerchantTheId)
        .then(function (response) {
          ...
          self.choseText = response.data.merFeeTemplateId
          self.choseText = response.data.merFeeTemplateName
        })
    ...
    pagByJson: function (pagJson) {
      ...
      this.formDataNew.merFeeTemplateId = pagJson.merFeeTemplateId
    ...
    handleSubmit () {
      ...
      this.$refs['formDataNew'].validate((valid) => {
        if (valid) {
          this.$axios.put('/channelMerchant', this.formDataNew).then(function (res) {
