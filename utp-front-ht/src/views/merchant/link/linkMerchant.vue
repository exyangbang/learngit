<template>
    <div>
            ...
            <Col>
                <Form :label-width="100" ref="form" :model="formData" label-position="right" :inline="true">
                    ...
                    <FormItem :label-width="0">
                      ...
                      <Button type="success" style="position: relative; left: -100px" @click="createBtn()" v-access id="addMerchant">新增关联</Button>
                    </FormItem>
                </Form>
            </Col>
        </Row>
        <Row>
            <Col>
            <Table ref="selection" :columns="tableColumns" :data="tableData" highlight-row>
      ...
      <!--<Modal v-model="funcModal" title="新增关联商户" width="1000" :mask-closable="false">-->
        <!--<func-list v-if="funcModalShow" :merId="merId" :merName="merName" :merNo="merNo" :productNoAry="productNoAry" :chnFeeTemplateIdAry="chnFeeTemplateIdAry">-->
        <!--</func-list>-->
...
<script>export default {
  ...
  methods: {
    ...
    doDelete (row) {
      let self = this
      this.$axios.delete('/channelMerchant/' + row.id)
        .then(function () {
    ...
    searchList (param) {
      ...
      // param.merName = this.$route.params.merName
      param.parameters = {}
      if (!this.$route.params.merNo) {
        param.parameters.merNo = window.location.href.split('/')[(window.location.href.split('/').length - 1)]
      } else {
        param.parameters.merNo = this.$route.params.merNo
      }
      // 获取 商户列表(证书关联的商户)
      this.$axios.post('channelMerchant/list', param)
        .then((response) => {
