...
<template>
  <card>
    <Form  :model="formData"  ref="formData" :label-width="120" :rules="ruleValidate">
      <Row>
        <Col span="10" offset="" >
          <h4 style="display:inline">添加商户</h4>
          ...
          <row>
            ...
            <Col span="12">
              <FormItem label="商户号：" prop="merNo">
                <Input v-model="formData.merNo" style="width: 300px" placeholder="请输入商户号" :maxlength="20" />
              </FormItem>
            </Col>
          </row>
          <row>
  ...
  data () {
    return {
      ...
      ruleValidate: {
        ...
        orgCode: [
          {required: true, max: 20, trigger: 'blur', message: '请选择所属收单机构'},
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          this.$axios.post('/merchantInfo/add', this.formData).then(function (res) {
