...
export default {
  ...
  data () {
    return {
      ...
      ruleValidate: {
        accountNo: [
          {required: true, trigger: 'blur', message: '请输入来款账号'},
  ...
  activated () {
    let initParamVal = this.$util.getRefreshInitParam(this, 'editIncomingId', this.$route.params.id)
    let industryCodeVal = this.$util.getRefreshInitParam(this, 'editIncomingIndustryCode', this.$route.params.industryCode)
    if (this.id !== initParamVal) {
      this.id = initParamVal
      this.industryCode = industryCodeVal
      this.init()
    }
  }
