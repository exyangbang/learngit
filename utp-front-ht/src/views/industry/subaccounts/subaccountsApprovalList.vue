      ...
      <Col>
        <Form :label-width="120" ref="form" :model="formData" label-position="right" :inline="true">
          ...
          <FormItem label="客户名称：" prop="name">
            <Input v-model="name" :maxlength="12"/>
          </FormItem>
...
export default {
  ...
  methods: {
    ...
    searchList (param) {
      ...
      let uri = encodeURI('/subaccountsmtALimitApproval/list')
      ...
      if (this.transEndDatePass) {
        if (this.endTransTime) {
          ...
          this.transEndDate = this.transEndDatePass + ' '+ '00:00:00'
        }
        query.parameters.transEndDate = this.transEndDate
      } else {
        delete query.parameters.transEndDatePass
      }
      // 查询机构列表
      // let that = this
    ...
    sumbitAuditStatus () {
      ...
      if (this.auditStatus && this.id) {
        ...
        this.$axios.put('/subaccountsmtALimitApprovalAudit', formData).then(function (res) {
          that.auditStatus = ''
          that.id = ''
          that.remark = ''
          that.searchList()
          that.$Message.success('子账户交易限额审核成功')
