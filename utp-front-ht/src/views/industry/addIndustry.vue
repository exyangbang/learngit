...
<template>
  <card>
    <Form  :model="formData"  ref="formData" :label-width="150" :rules="ruleValidate">
      <Row>
        <Col span="10" offset="" >
          <h4 style="display:inline">添加客户</h4>
          ...
          <row>
            <Col span="12">
              <FormItem label="所属机构：" prop="orgCode">
                <i-select v-model="formData.orgCode" @on-change="orgChange" :label-in-value = "true" style="width: 300px" placeholder="请选择机构" >
...
<!--                collectionAccountChange-->
                <i-select v-model="formData.collectionIndustryId"  style="width:300px"  placeholder="请选择手续费归集账户">
            ...
            <Col span="12">
              <FormItem label="平台上账条件：" prop="industryIncomeType">
                <i-select v-model="formData.industryIncomeType"  style="width:300px" @on-change="industryIncomeTypeChange" placeholder="请选择平台上账条件">
            ...
            <Col span="12">
              <FormItem label="鉴权方式：" prop="authenticationMode">
                <i-select v-model="formData.authenticationMode"  style="width:300px" @on-change="authenticationModeChange" placeholder="请选择认证要素">
          ...
          <row>
            <Col span="12">
              <FormItem label="是否对公打款验证：" prop="needPublicAuthAmt">
                <i-select v-model="formData.needPublicAuthAmt"  style="width:300px" @on-change="needPublicAuthAmtChange" placeholder="请选择是否对公打款验证">
                  <i-option value="1">是</i-option>
                  <i-option value="0">否</i-option>
                </i-select>
              </FormItem>
          ...
          <row>
            <Col span="12">
              <FormItem label="开通业务：" prop="businessType">
                <i-select v-model="formData.businessType"  style="width:300px" @on-change="businessTypeChange" placeholder="请选择开通业务">
          ...
          <row>
            <Col span="12" v-if="maxBrokerageRateGo">
              <FormItem label="是否开通双账户：" prop="isOpneDoubleAcc">
                <i-select v-model="formData.isOpneDoubleAcc"  style="width:300px" @on-change="isOpneDoubleAcc" placeholder="请选择是否开通双账户">
                  <i-option value="1">是</i-option>
          ...
          <row>
            <Col span="12" v-if="formData.isOpneBalancePay==='1' && maxBrokerageRateGo"><!-- “T0”、“T1”，默认值“T1”isOpneBalancePayGo-->
              <FormItem label="余额支付账期：" prop="balancePayPeriod">
                <i-select v-model="formData.balancePayPeriod"  style="width:300px" @on-change="balancePayPeriod" placeholder="请选择余额支付账期">
          ...
          <row>
            ...
            <Col span="12" v-if="maxBrokerageRateGo">
              <FormItem label="开户模式：" prop="openAccountMode">
                <i-select v-model="formData.openAccountMode"  style="width:300px" placeholder="请选择开户模式">
...
export default {
  ...
  data () {
    return {
      ...
      accountFeeExpenditure: true,
      accountFeeProfit: true,
      ...
      ruleValidate: {
        ...
        bankAccountNo: [
          {required: true, trigger: 'change', message: '请选择资金账号'}
        ],
        bankCode: [
          {required: false, trigger: 'blur', message: '请输入清算行号'}
          // {pattern: /^[0-9]*$/, message: '请输入客户账户！'} ^(?=.*?[A-Z])(?=.*?[^A-Za-z0-9]).{6,12}$
        ],
        code: [
        ...
        authPaymentAccountId: [
          {required: false, trigger: 'change', message: '请选择小额打款支出账户'}
        ],
        ableBindMultiCard: [
  ...
  methods: {
    init () {
      ...
      //     if (e.industryType === '6') {
      //       self.collectionAccountAccountAry.push(e)
      //     }
      //   })
      // })
      ...
      this.$axios.post('/fundAccount/list', query1).then(function (res) {
        self.custAccountNoAry = res.data.list
      })
    },
    industryIncomeTypeChange: function(value) {
      let self = this
      if (value === '1') {
        self.$axios.get('industry/exist/' + self.formData.fundAccount).then(function (res) {
          self.whetherExternalDis = true
          self.formData.whetherExternal = '0'
        }).catch(function (error) {
          self.formData.industryIncomeType = null
        })
      } else {
        this.whetherExternalDis = false
      }
    },
    ...
    needPublicAuthAmtChange: function (val) {
      ...
      if (val === '1') {
        ...
        this.formData.authPaymentAccountId = null // 小额打款支出账
      }
    },
    ableBindMultiCardChange: function (val) {
      this.formData.ableBindMultiCard = val
    },
    isOpneBalancePay: function (val) {
    ...
    custAccountNoChange: function (val) {
      ...
      this.$axios.get('/fundAccount/' + idNew).then(function (res) {
        ...
        self.smallQuery.parameters.fundAccount = self.formData.fundAccount
        self.$axios.post('/industry/pageListFeeAndAuthAccount', self.smallQuery).then(function (res) {
          if (res.data.list.length === 1) {
            ...
            self.formData.authPaymentAccountId = res.data.list[0].id
          } else {
            self.authPaymentAccountAry = res.data.list
          }
        })
    ...
    //   this.formData.collectionAccount = val
    // },
    handleSubmit () {
      let that = this
