      ...
      <Col>
        <Form :label-width="120" ref="form" :model="formData" label-position="right" :inline="true">
          <FormItem label="客户名称：" prop="industryName">
            <Input v-model="industryName" :maxlength="200"/>
          </FormItem>
...
export default {
  ...
  methods: {
          ...
          render: (h, params) => {
            if (params.row.auditStatus !== '0') {
              return h('div', [
                ...
                h('svg-icon', {
                  attrs: {
                    id: 'quotaExaApprovalListShuaXin',
                    title: '刷新额度'
                  },
                  props: {
                    type: 'reset-pwd',
    ...
    searchList (param) {
      ...
      let uri = encodeURI('/industryAmLimit/list')
      ...
      if (this.industryName) {
        ...
        delete query.parameters.industryName
      }
    ...
    doDelete (row) {
      ...
      row.operationType = '0'
      this.$axios.post('/industryAmLimitApproval', row)
        .then((res) => {
          if (res.status === 200) {
