      ...
      <Col>
        <Form :label-width="120" ref="form" :model="formData" label-position="right" :inline="true">
          ...
          <FormItem :label-width="0">
            <Button type="primary" v-access id="whiteListPaymentsList" @click="searchBtn()" style="margin-right: 10px;">查询</Button>
...
export default {
  ...
  methods: {
    ...
    searchList (param) {
      ...
      let uri = encodeURI('/whiteListPayments/list')
    ...
    doDelete (row) {
      row.operationType = '2'
      this.$axios.post('/whiteListPaymentsAudit', row)
        .then((res) => {
          if (res.status === 200) {
