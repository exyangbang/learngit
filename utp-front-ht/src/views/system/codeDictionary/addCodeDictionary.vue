...
<template>
  <card>
    <Form  :model="formData"  ref="formData" :label-width="120" :rules="ruleValidate">
      <Row>
        <Col span="10" offset="" >
          <h4 style="display:inline">新增数据字典</h4>
      ...
      <Row>
        <Col span="24">
          <row>
            <Col span="12">
              <FormItem label="类型代码：" prop="type">
                <Select v-model="formData.type" style="width: 180px"  @on-change="objTypeChange">
            ...
            <Col span="12">
              <FormItem label="类型名称：" >
                <Input v-model="formData.typeName" style="width: 300px" placeholder="请输入类型名称" :maxlength="200" disabled="none"/>
              </FormItem>
            </Col>
          </row>
          <row>
            <Col span="12">
              <FormItem label="子项目编码：" prop="code">
                <Input v-model="formData.code" style="width: 300px" placeholder="请输入子项目编码" :maxlength="100" />
              </FormItem>
            </Col>
            <Col span="12">
  ...
  methods: {
    ...
    handleSubmit () {
      ...
      this.$refs['formData'].validate((valid) => {
        if (valid) {
          this.$axios.post('/codeDictionary', this.formData).then(function (res) {
            that.$Message.success('添加数据字典成功')
            that.goBack()
          })
        } else {
