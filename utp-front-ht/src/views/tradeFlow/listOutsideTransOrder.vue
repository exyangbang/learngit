  ...
  <card>
    <Form :label-width="120" ref="form" :model="formData" label-position="right" :inline="true">
      <Row>
        ...
        <FormItem label="支付订单号：" prop="transOrderId">
          <Input v-model="transOrderId" :maxlength="accountSize" class="input-width-170"/>
        </FormItem>
    ...
    <Modal title="查看图片" v-model="visible">
      <img :src="this.constantsImg" style="width: 100%">
      <!--<p v-if="disTex">未上传图片</p>-->
    </Modal>
  </card>
</template>
<script>export default {
  name: "listOutsideTransOrder",
  data () {
    return {
      goodsInfo: '',
      visible: false,
  ...
  methods: {
    ...
    init () {
      ...
      this.tableColumns = [
        ...
        },
        {
          ...
          render: function (h, a, b) {
            let getReslt = ''
            if (a.row.discountAmount) {
              getReslt = a.row.discountAmount / 100
            }
            return h('span', getReslt + '元')
          }
        },
        ...
        },
        {
          ...
          render: function (h, a, b) {
            let getReslt = ''
            if (a.row.splitStatus === '0') {
              getReslt = '未分账'
            }
            if (a.row.splitStatus === '1') {
              getReslt = '已分账'
            }
            if (a.row.splitStatus === '2') {
              getReslt = '部分分账'
            }
            return h('span', getReslt)
          }
        },
        {
          ...
          render (h, a, b) {
            if (a.row.payMode === '3') {
              ...
              // return h('div', a.row.remark.size > 30 ? a.row.remark : a.row.remark)
              return h('div', [
                h('span', {
                  style: {
    ...
    //   this.visible = true
    //   let self = this
    //   this.$axios.get('/outsideTransOrder/' + row.id).then(function (res) {
    //     self.constantsImg = res.data.remark
    //     // self.constantsImg = 'data:image/pdf;base64,' + res.data.remark
    //     debugger
    //   })
    // },
    ...
    downloadGo (data) {
      ...
      let downName = '上送支付流水.xlsx' // 文件下载名称
      const blob = new Blob([data])
      if (window.navigator.msSaveOrOpenBlob) {
        // 兼容IE10
        navigator.msSaveBlob(blob, downName)
        ...
        link.style.display = 'none'
        link.href = url
        link.setAttribute('download', '上送支付流水' + '.xlsx')
        document.body.appendChild(link)
        link.click()
      }
    },
