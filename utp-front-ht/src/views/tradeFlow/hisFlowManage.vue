...
<template>
  <card>
      ...
      <Row>
        ...
        <FormItem label="交易状态：" prop="transStatus">
          <Select v-model="transStatus" class="input-width-170" @on-change="chanStatus">
            ...
            <!--<Option value="SPLIT_PAYMENT"> 分账</Option>-->
          </Select>
        </FormItem>
    ...
    <Row>
      <Table :columns="tableColumns" :data="tableData" @on-current-change="selectDataGo" border highlight-row stripe></Table><br/>
      <Page :total="total" :current="pageNo" show-total @on-change="changePage" class="page"/>
    </Row>
    <Modal
      title="详情"
      v-model="hisFlowManageOrderDetail"
      @on-ok="thawReportOk"
      :mask-closable="false">
      <p>交易平台订单号：{{orderId}}</p>
      <br>
...
<script>export default {
  ...
  data () {
    return {
      ...
      totalCount: '',
      totalMoney: '',
      hisFlowManageOrderDetail: false,
      selectDataId: '',
      disAction: true,
      isHidden: true,
      productNoAry: [],
  ...
  methods: {
    ...
    accountReportBank () {
      this.hisFlowManageOrderDetail = true
    },
    thawReportOk () {
      this.hisFlowManageOrderDetail = false
    },
    searchOrg () {
      let self = this
    ...
    //   link.style.display = 'none'
    //   link.href = url
    //   link.setAttribute('download', '历史交易流水' + '.xlsx')
    //   document.body.appendChild(link)
    //   link.click()
    // },
    searchList () {
      ...
      //   this.beginTransTime = this.beginTransTime.split(':')
      //   let newTime = this.beginTransTime.join('')
      //   this.formData.parameters.beginTransTime = newTime
      // } else {
      ...
      //   this.endTransTime = this.endTransTime.split(':')
      //   let newTime = this.endTransTime.join('')
      //   this.formData.parameters.endTransTime = newTime
      // } else {
      ...
      this.formData.pageSize = self.pageSize
      let query = self.formData
      this.$axios.post('/transFlowHistory/list', query)
        .then(function (response) {
    ...
    downloadGo (data) {
      ...
      let downName = '历史交易流水.xlsx' // 文件下载名称
      const blob = new Blob([data])
      if (window.navigator.msSaveOrOpenBlob) {
        // 兼容IE10
        navigator.msSaveBlob(blob, downName)
