...
<script>export default {
  ...
  methods: {
    ...
    downloadGo (data) {
      ...
      let downName = '子账户分账信息详情.xlsx' // 文件下载名称
      const blob = new Blob([data])
      if (window.navigator.msSaveOrOpenBlob) {
        // 兼容IE10
        navigator.msSaveBlob(blob, downName)
        ...
        link.style.display = 'none'
        link.href = url
        link.setAttribute('download', '子账户分账信息详情' + '.xlsx')
        document.body.appendChild(link)
        link.click()
      }
    },
  ...
  activated () {
    let initParamVal = this.$util.getRefreshInitParam(this, 'subAccountSettleSplitDetail', this.$route.params.transOrderId)
