...
<script>export default {
  ...
  data () {
    return {
      id: this.$route.params.transOrderId,
      payerAccount: '',
      disBtn: true,
      totalCount: '',
      totalMoney: '',
  ...
  methods: {
    ...
    init () {
      ...
      // if (this.$route.params.transOrderId) {
      //   if (this.$route.params.transOrderId !== sessionStorage.getItem('listSplitInfoDetail.transOrderId')) {
      //     sessionStorage.setItem('listSplitInfoDetail.transOrderId', this.$route.params.transOrderId)
      //   }
      // } else {
      //   this.$route.params.transOrderId = sessionStorage.getItem('listSplitInfoDetail.transOrderId')
      // }
      if (this.id) {
        this.searchList()
      }
    ...
    searchList () {
      ...
      this.formData.pageNo = self.pageNo
      this.formData.pageSize = self.pageSize
      // self.formData.parameters.transOrderId = this.id
      self.formData.parameters.flowTransOrderId = this.id
    ...
    downloadGo (data) {
      ...
      let downName = '子账户单笔付款信息详情.xlsx' // 文件下载名称
      const blob = new Blob([data])
      if (window.navigator.msSaveOrOpenBlob) {
        // 兼容IE10
        navigator.msSaveBlob(blob, downName)
        ...
        link.style.display = 'none'
        link.href = url
        link.setAttribute('download', '子账户单笔付款信息详情' + '.xlsx')
        document.body.appendChild(link)
        link.click()
      }
    },
  ...
  activated () {
    let initParamVal = this.$util.getRefreshInitParam(this, 'subAccountPaymentDetail', this.$route.params.transOrderId)
